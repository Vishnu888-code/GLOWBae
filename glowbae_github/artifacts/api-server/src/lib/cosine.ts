...
export function encodeQuery(
  ...
  const priceNorm = 1;
  return { skinTypeVec, concernVec, priceNorm: budget };
}
...
export function rankProducts(
  ...
  const queryEnc = encodeQuery(query.skinType, query.concern, query.budget);
  const queryVec = buildFeatureVector(
    queryEnc.skinTypeVec,
    queryEnc.concernVec,
    1
  );
  ...
  const scored = products
    ...
    .map((p) => {
      ...
      if (skinMatch)
        matchReasons.push(`Formulated for ${query.skinType} skin`);
      if (concernMatch)
        matchReasons.push(`Targets ${query.concern}`);
      if (p.price <= query.budget * 0.6)
        matchReasons.push("Great value for money");
      if (!skinMatch && !concernMatch)
        matchReasons.push("Universally beneficial formula");
